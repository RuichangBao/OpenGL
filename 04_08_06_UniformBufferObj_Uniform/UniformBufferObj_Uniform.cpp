...
//Uniform缓冲对象
#include "UniformBufferObj_Uniform.h"
#include <glad/glad.h>
...
int main()
{
	...
	Shader planShader("shader/PlanVertex.shader", "shader/PlanFragment.shader");
	// cube VAO
...
#pragma region Uniform缓冲对象
	// 获取uniform块的索引
	GLuint uniformIndexCube = glGetUniformBlockIndex(shader.ID, "Matrices");
	GLuint uniformIndexPlan = glGetUniformBlockIndex(planShader.ID, "Matrices");
	// 将UBO绑定到绑定点
	glUniformBlockBinding(shader.ID, uniformIndexCube, 0);
	glUniformBlockBinding(planShader.ID, uniformIndexPlan, 0);
